...
export function Optimize(props: OptimizeProps): JSX.Element {
    const optimizer = async () => {
        ...
        for (let i = 0; i < props.deliveryMap.length; i++) {
            ...
            await fetch('http://730335660517.dkr.ecr.us-east-1.amazonaws.com/swiftwing-backend:5000/optimize/optimizeroute', { 
            method: 'POST',
            headers: {
    ...
    const pushOptimized = async () => {
        ...
        console.log(payload);
        await fetch('http://730335660517.dkr.ecr.us-east-1.amazonaws.com/swiftwing-backend:5000/delivery/update_delivery/', { 
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
            'Access-Control-Allow-Origin': '*',
        ...
        for (let i = 0; i < props.deliveryUser.length; i++) {
            if(props.deliveryUser[i] != "00-unassigned"){
                ...
                for (let j = 0; j < delivery_ids.length; j++) {
                    ...
                    console.log(payload);
                    await fetch('http://730335660517.dkr.ecr.us-east-1.amazonaws.com/swiftwing-backend:5000/shipping/update_shipping_status_and_driver/',{
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'Access-Control-Allow-Origin': '*',
